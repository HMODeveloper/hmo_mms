...
async def get_search_info_handler(
    ...
    for level in UserLevel:
        levels_info.append(UserLevelInfo(level=level.value, code=level.name))
...
async def search_handler(
    ...
    sensitivePermission = user.has_permission(UserLevel.ADMIN)
    ...
    try:
        ...
        if request.create_at_start is not None and request.create_at_end is not None:
            filters.append(
                User.create_at.between(request.create_at_start, request.create_at_end)
            )
        ...
        if request.departments:
                ...
                (
                    await db.execute(
                        select(Department).where(
                            Department.code.in_(request.departments)
                        )
                    )
                )
                .scalars()
            ...
            filters.append(
                User.id.in_(
                    select(user_department_association.c.user_id).where(
                        user_department_association.c.department_id.in_(department_ids)
                    )
                )
            )
...
async def get_member_profile_handler(
    ...
    sensitivePermission = current_user.has_permission(UserLevel.ADMIN)
    ...
    for department in user.departments:
        departments.append(DepartmentInfo(name=department.name, code=department.code))
